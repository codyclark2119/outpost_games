    ...
    <div class="container mx-auto px-4">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            <router-link to="/products" class="text-outpost-gold hover:text-outpost-gold-dark text-sm font-medium flex items-center gap-1 mb-2">
            ...
            <p class="text-gray-500 mt-1">{{ filteredProducts.length }} product{{ filteredProducts.length !== 1 ? 's' : '' }} available</p>
          </div>
        </div>
        ...
        <div v-else-if="!currentType" class="text-center py-20">
          ...
          <router-link to="/products" class="btn-primary px-6 py-2">← Back to Products</router-link>
        </div>
        ...
        <template v-else>
          <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-56 flex-shrink-0">
              <div class="bg-white rounded-xl shadow border border-gray-200 p-5 sticky top-24">
                ...
                <div class="mb-5">
                  ...
                  <select v-model="sortBy" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-outpost-navy">
                ...
                <!-- Filter by set -->
                <div>
                  <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Sets</label>
            ...
            <div class="flex-1 min-w-0">
              ...
              <div v-else-if="filteredProducts.length === 0" class="text-center py-16 text-gray-400">
                <p class="text-lg font-medium">No products match your filters.</p>
                <button class="btn-primary px-5 py-2 mt-4 text-sm" @click="clearFilters">Clear Filters</button>
              </div>
              ...
              <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div class="p-4 flex flex-col flex-grow">
                    <!-- Set badge -->
                    <span class="text-xs text-outpost-gold font-semibold bg-outpost-gold/10 px-2 py-0.5 rounded-full inline-block mb-2 self-start">
                      {{ item.setName }}
                    </span>
                    <h3 class="font-cinzel font-bold text-gray-800 mb-1 leading-tight">{{ item.product.name }}</h3>
                    <p v-if="item.product.description" class="text-sm text-gray-500 mb-3 flex-grow line-clamp-3">{{ item.product.description }}</p>
                    <div v-if="item.product.price !== null" class="mt-auto pt-2 border-t border-gray-100">
                      <span class="text-xl font-bold text-outpost-navy">${{ Number(item.product.price).toFixed(2) }}</span>
                    </div>
                  </div>
                </div>
              </div>
...
const filteredProducts = computed((): ProductItem[] => {
  ...
  if (maxPrice.value !== null) {
    items = items.filter(i => i.product.price !== null && i.product.price <= (maxPrice.value ?? Infinity))
  }
  ...
  return [...items].sort((a, b) => {
    switch (sortBy.value) {
      ...
      case 'price-asc':  return (a.product.price ?? Infinity) - (b.product.price ?? Infinity)
      case 'price-desc': return (b.product.price ?? -Infinity) - (a.product.price ?? -Infinity)
